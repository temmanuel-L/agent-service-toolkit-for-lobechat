...
def create_rag_system_prompt(
    ...
    if not kb_ids:
        ...
        # 简化但保留关键检索流程约束：何时必须查 KB、何时应走长期记忆、避免无意义重复检索。
        tool_instruction = """
**检索流程规则**：
1. 当用户询问知识库文档中的**具体事实**（如合同甲方/乙方、公司名、金额、日期、条款、附件内容、论文/报告的作者/标题/期刊名等）时，必须先调用 `search_knowledge` 检索，再根据检索结果回答，不能仅依赖长期记忆。
2. 对于显然是关于**用户本人或日常生活偏好**的问题（例如“我是谁”“我喜欢吃什么”“我最近都在干什么”等），优先使用对话上下文和长期记忆回答，一般不要调用 `search_knowledge`，除非用户明确要求查某个文档。
3. 每轮对话中，针对同一个问题，`search_knowledge` 通常只调用一次；只有当用户补充了关键信息或明确要求“重新检索/更换知识库”时，才再次调用。"""
...
**回答原则**：
1. **上下文优先**：把 `search_knowledge` 返回的内容和对话上下文作为主要信息源。回答必须与这些内容保持一致，不得引入与上下文明显矛盾的说法。
2. **全面但不过度复制**：尽量覆盖所有与问题直接相关的要点，必要时可以引用关键句子、数字或条款编号，但不要大段逐字粘贴原文。
3. **结构清晰**：先给出直接结论，再用分点或小节的方式展开说明；对于合同、规范、报告类问题，优先按“模块 / 条款 / 要点”来组织内容。
4. **事实优先 & 拒绝模糊**：如果上下文中已经给出了公司名、人名、时间、金额、条款编号等具体信息，应直接写出，避免使用“某公司”“未明确”等含糊说法来代替已有事实。
5. **信息缺失要说明白**：如果检索结果中没有用户想要的关键信息，应明确说明“在当前知识库内容中未查到相关信息”，不要凭常识或想象补齐。
6. 始终用{user_language}输出。"""
